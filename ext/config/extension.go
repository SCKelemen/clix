...
func (Extension) Extend(app *clix.App) error {
	...
	if findSubcommand(app.Root, "config") == nil {
...
func configResetCommand(app *clix.App) *clix.Command {
	...
	cmd.Run = func(ctx *clix.Context) error {
		if !force {
			answer, err := app.Prompter.Prompt(ctx.Context, clix.PromptRequest{Label: "Reset configuration? (y/N)", Theme: app.DefaultTheme})
			if err != nil {
				return err
			}
